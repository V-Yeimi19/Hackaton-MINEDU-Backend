...
- **Jerarquía**: `Institution` (IE, solo la crea el `DIRECTIVO`, puede tener varias) → `Classroom` (aula, la crea un `DOCENTE` de la IE; `gradeLevel` vive aquí ahora) → `Course` (curso dentro del aula — relación invertida respecto al modelo viejo).
