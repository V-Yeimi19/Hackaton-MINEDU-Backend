...
- **Roles** (`Role` enum en `@minedu/common`, remodelado 2026-07-25): `ADMIN`, `DIRECTIVO`, `DOCENTE`, `FAMILIAR`. Cada endpoint declara `@Roles(...)` + `RolesGuard`. Ya no existen `ESPECIALISTA` ni `ESTUDIANTE` — el estudiante no es usuario (es un registro `Student` en `classroom_db`, creado y gestionado por su `FAMILIAR`). El `DIRECTIVO` crea Instituciones Educativas e invita `DOCENTE`s; el `DOCENTE` crea aulas e invita `FAMILIAR`es (1 hijo por link de invitación). Varios controladores aún referencian los roles muertos — trabajo de adaptación en curso, ver [PENDING.md](./PENDING.md).
